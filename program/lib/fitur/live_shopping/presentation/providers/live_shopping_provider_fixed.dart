...
class LiveShoppingNotifier extends StateNotifier<LiveShoppingState>
  ...
  // ✅ FUNGSI YANG DIPERBAIKI: Join room dengan struktur data yang konsisten
  Future<void> joinRoom({
    ...
    try {
      ...
      // ✅ PERBAIKAN: Handle broadcaster specific setup dengan struktur data yang konsisten
      if (role == 'broadcaster') {
        debugPrint("Setting up Firestore for broadcaster...");
        
        // STRUKTUR BARU: Simpan dengan roomId sebagai document ID agar mudah di-query
        await _firestore.collection('live_sessions').doc(roomId).set({
          'hostId': userId,           // ✅ Firebase Auth UID jastiper
          'hostName': username,      // ✅ Nama jastiper
          'roomId': roomId,          // ✅ Room ID dari 100ms
          ...
          // ✅ TAMBAHAN: Metadata untuk debugging dan analytics
          'sessionType': 'live_shopping',
          'platform': 'mobile_app',
          'viewerCount': 0,
          'totalViews': 0,
        });
        
        // ✅ TAMBAHAN: Buat backup document dengan hostId sebagai document ID (untuk backward compatibility)
        await _firestore.collection('live_sessions').doc(userId).set({
          'hostId': userId,
          'hostName': username,
          'roomId': roomId,          // ✅ PENTING: Simpan roomId di sini juga
          'title': liveTitle ?? 'Live Shopping',
          'status': 'ongoing',
          'itemPrice': initialPrice ?? 0.0,
          'createdAt': FieldValue.serverTimestamp(),
          'updatedAt': FieldValue.serverTimestamp(),
          'isBackupDoc': true,       // ✅ Flag bahwa ini backup document
          'primaryDocId': roomId,    // ✅ Reference ke document utama
        });
        
        debugPrint("✅ Firestore setup completed - Primary doc: $roomId, Backup doc: $userId");
      }
  ...
  void updateAndBroadcastPrice(double price) {
    ...
    // ✅ TAMBAHAN: Update price di Firestore juga
    if (state.currentRole == 'broadcaster' && state.roomId != null) {
      _firestore.collection('live_sessions').doc(state.roomId!).update({
        'itemPrice': price,
        'updatedAt': FieldValue.serverTimestamp(),
      }).catchError((e) => debugPrint("Error updating price in Firestore: $e"));
    }
  }
  ...
  // ✅ FUNGSI YANG DIPERBAIKI: Leave room dengan cleanup Firestore yang benar
  Future<void> leaveRoom() async {
    ...
    try {
      ...
      // ✅ STEP 3: Update Firestore status dengan struktur yang benar
      if (userIsBroadcaster && roomId != null) {
        debugPrint("Updating Firestore status...");
        
        // Update primary document (roomId)
        await _firestore.collection('live_sessions').doc(roomId).update({
          'status': 'ended',
          'endedAt': FieldValue.serverTimestamp(),
          'updatedAt': FieldValue.serverTimestamp(),
        }).catchError((e) => debugPrint("Error updating primary live session: $e"));
        
        // Update backup document (userId) jika ada
        if (userId != null) {
          await _firestore.collection('live_sessions').doc(userId).update({
            'status': 'ended',
            'endedAt': FieldValue.serverTimestamp(),
            'updatedAt': FieldValue.serverTimestamp(),
          }).catchError((e) => debugPrint("Error updating backup live session: $e"));
        }
      }
  ...
  void onJoin({required HMSRoom room}) {
    ...
    // ✅ TAMBAHAN: Update viewer count jika user adalah viewer
    if (state.currentRole != 'broadcaster' && state.roomId != null) {
      _updateViewerCount(state.roomId!, 1); // +1 viewer
    }
  }

  // ✅ FUNGSI BARU: Update viewer count
  Future<void> _updateViewerCount(String roomId, int increment) async {
    try {
      await _firestore.collection('live_sessions').doc(roomId).update({
        'viewerCount': FieldValue.increment(increment),
        'totalViews': FieldValue.increment(increment > 0 ? 1 : 0),
        'updatedAt': FieldValue.serverTimestamp(),
      });
      debugPrint("Viewer count updated: +$increment");
    } catch (e) {
  ...
  void onPeerUpdate({required HMSPeer peer, required HMSPeerUpdate update}) {
    ...
    if (update == HMSPeerUpdate.peerLeft && !peer.isLocal) {
      ...
      if (peer.role?.name == 'broadcaster' || peer.role?.name == 'host') {
        ...
        // ✅ TAMBAHAN: Update viewer count jika viewer yang keluar
        if (state.currentRole == 'broadcaster' && state.roomId != null) {
          _updateViewerCount(state.roomId!, -1); // -1 viewer
        }
      }
  ...
  void onRemovedFromRoom({required HMSPeerRemovedFromPeer hmsPeerRemovedFromPeer}) {
    ...
    // ✅ TAMBAHAN: Update viewer count jika user di-kick
    if (state.currentRole != 'broadcaster' && state.roomId != null) {
      _updateViewerCount(state.roomId!, -1);
    }
  }
  @override
  ...
  void onPeerListUpdate({required List<HMSPeer> addedPeers, required List<HMSPeer> removedPeers}) {
    // ✅ TAMBAHAN: Handle peer list update untuk viewer count
    if (state.currentRole == 'broadcaster' && state.roomId != null) {
      final viewerChange = addedPeers.length - removedPeers.length;
      if (viewerChange != 0) {
        _updateViewerCount(state.roomId!, viewerChange);
      }
    }
  }
  @override
  ...
  void dispose() {
    ...
    // ✅ TAMBAHAN: Update viewer count saat dispose jika user adalah viewer
    if (state.currentRole != 'broadcaster' && state.roomId != null) {
      _updateViewerCount(state.roomId!, -1);
    }
...
// ✅ PROVIDER YANG DIPERBAIKI: Query berdasarkan status dan roomId
final liveSessionsStreamProvider = StreamProvider.autoDispose<QuerySnapshot>((ref) {
  return FirebaseFirestore.instance
      .collection('live_sessions')
      .where('status', isEqualTo: 'ongoing')
      .where('isBackupDoc', isEqualTo: false) // ✅ Exclude backup documents
      .orderBy('createdAt', descending: true)
      .snapshots();
});

// ✅ PROVIDER BARU: Query live session berdasarkan roomId
final liveSessionByRoomIdProvider = StreamProvider.autoDispose.family<DocumentSnapshot?, String>((ref, roomId) {
  return FirebaseFirestore.instance
      .collection('live_sessions')
      .doc(roomId)
      .snapshots();
...
// ✅ PROVIDER BARU: Query live session berdasarkan hostId (backward compatibility)
final liveSessionByHostIdProvider = StreamProvider.autoDispose.family<DocumentSnapshot?, String>((ref, hostId) {
  return FirebaseFirestore.instance
      .collection('live_sessions')
      .doc(hostId)
      .snapshots();
